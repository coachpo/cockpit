...
## OVERVIEW
Cockpit is a meta-repo that pins the published `backend/` and `frontend/` repositories as submodules. Root-owned files stay limited to submodule wiring, cross-service startup helpers, shared container automation, and local-only scratch state that is gitignored.
